...
Config lives under a `wikiScitools` key inside the agent folder's `settings.local.json`, so it
coexists with the agent's own settings instead of fighting them.

## Lookup order

First file that contains a `wikiScitools` key wins:
...
for d in "${CLAUDE_PROJECT_DIR:-.}/.claude" "${CLAUDE_PROJECT_DIR:-.}/.opencode" \
  ...
  [ -f "$f" ] && grep -q '"wikiScitools"' "$f" && { echo "CONFIG: $f"; cat "$f"; break; }
done
```

Project scope beats user scope. Read the `wikiScitools` object; ignore every sibling key, they
belong to the agent, not to us.

**Legacy fallback.** If nothing is found, check the pre-`settings.local.json` locations and treat
the file as a flat config (no `wikiScitools` wrapper):
...
Two groups, and the difference matters. Most keys point **into your knowledge base**. One points
**next to the document you are working on**.

### Resolved against `knowledgeBase`

| Key | Meaning | Default |
|---|---|---|
| `knowledgeBase` | absolute path to the wiki root | **required** |
| `wikiDir` | compiled articles | `wiki` |
| `rawDir` | raw sources (PDFs, ...) | `raw` |
| `convertedDir` | permanent cache of converted sources | `converted` |

### Resolved against the draft

| Key | Meaning | Default |
|---|---|---|
| `reportsDir` | where `/wiki-check` writes its report | `reports` |
...
├── wiki/                          <- wikiDir         (read)
├── raw/                           <- rawDir          (read)
└── converted/                     <- convertedDir    (written, cached forever)
...
Top-level keys inside `wikiScitools` are defaults. `profiles.<name>` overrides any of them for that
profile. A profile with no entry is legal; it inherits every default.

Resolve in this order: `profiles.<name>.<key>` -> `wikiScitools.<key>` -> table default.
